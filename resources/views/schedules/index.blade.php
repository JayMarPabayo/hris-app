<x-layout>
    ...
    <section class="flex items-center gap-2 mb-4">
        ...
        <form method="GET" action="{{ route('schedules.index') }}" class="flex items-center gap-x-2">
            <input type="week" name="week" class="w-52" 
                   value="{{ $week ?? date('Y-\WW') }}" 
                   onchange="validateWeek(this)" required>
        ...
        {{-- Modal --}}
        <div id="validationModal" class="fixed inset-0 bg-gray-800 bg-opacity-50 items-center justify-center hidden">
            <div class="bg-white rounded-lg shadow-md p-6 w-96">
                <h2 class="text-lg font-medium mb-4 text-red-600">Empty Week Selected</h2>
                <p class="text-sm text-gray-700 mb-4">The selected week does not have any schedules. Please choose a valid week.</p>
                <div class="flex justify-end gap-2">
                    <button onclick="closeModal()" class="btn w-32">Close</button>
                </div>
            </div>
        </div>


        <div class="ms-auto relative flex gap-x-2 items-center text-white hover:text-teal-500 hover:scale-105 active:scale-95 duration-300">
                ...
                @forelse ($departmentSchedules as $index => $schedule)
                    ...
                    <tr class="data-row" title="{{ $schedule->shift->name }}">
                        <td class="max-w-36">
                            ...
                            <span class="block text-slate-400 truncate">
                                ...
                                @else   
                                    @if (is_null($selectedDay) || $selectedDay === $day)
                                        @if (in_array($day, $schedule->shift->weekdays)  ?? [])
                                            ...
                                            @endphp
                                            <p class="mb-1 text-slate-200">{{ $timePeriod }}</p>
    ...
    <script>
        const validWeeks = @json($weeksWithSchedules);

        function validateWeek(input) {
            const modal = document.getElementById('validationModal');
            
            if (!validWeeks.includes(input.value)) {
